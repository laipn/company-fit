...
There are legitimate processes to resolve these scenarios (certainly courts do all the time), but companies themselves (AFAIK) do not actual provide transparently their own internal decision-making.  There is no reason to not transparently document the procedure other than the legal (financial) liabilities in doing so. But this conscious lack of transparency in itself is an ethical decision that was made. There is no real moral justification for "hiding everything" -- even government military secrets have working [rules](https://en.wikipedia.org/wiki/Freedom_of_Information_Act_(United_States)) for this.  If the secret can't be made transparent, then at least the rules governing why its secret can be.  Corporation tend to demonstrate neither.
